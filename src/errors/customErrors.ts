...
export function unprocessableEntity(entity?: string | Array<string>): CustomError {
  ...
  return new CustomError('unprocessableEntity', message, httpStatus.UNPROCESSABLE_ENTITY);
}
export function internalServerError(message: string | void): CustomError {
  if (message == null) message = 'internal server error';
  return new CustomError('internalServerError', message, httpStatus.INTERNAL_SERVER_ERROR);
}
const customErrors = {
  badRequest,
  unauthorized,
